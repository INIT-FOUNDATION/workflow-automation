...
export const workflowRepository = {
    ...
    getTaskList: async(workflowId: Number) : Promise<any> => {
        try {
            const _queryForGetWorkflowTasks = {
                text: WORKFLOW.getTaskList,
                values: [workflowId]
            };

            const result = await pg.executeQueryPromise(_queryForGetWorkflowTasks);
            logger.debug(`workflowRepository :: getTaskList :: db result :: ${JSON.stringify(result)}`);
            
            return result;
        } catch (error) {
            logger.error(`workflowRepository :: getTaskList :: ${error.message} :: ${error}`)
            throw new Error(error.message);
        }
    },
