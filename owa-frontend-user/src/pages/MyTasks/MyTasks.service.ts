...
const getUserListbyDepId = async(departmentId: number) =>{
  const usersList = await get(`/api/v1/admin/users/list/department/${departmentId}`);
  return  usersList;
}

const createAssignment = async (data: any) => {
  const response = await get("api/v1/workflow/assignment/create", data);
  return response;
};

export { getWorkflowList, getTaskListByWorkflowId, getDepartmentList, getUserListbyDepId, createAssignment };
