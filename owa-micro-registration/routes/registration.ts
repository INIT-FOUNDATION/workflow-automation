...
registrationRouter.get("/health", async (req: Request, res: Response) => {
  try {
    return res.status(STATUS.OK).send("Registration Service is Healthy");
  } catch (error) {
    logger.error("Registration :: healthCheck :: ", error);
    return res
