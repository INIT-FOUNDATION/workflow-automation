...
import { STATUS, redis, generateToken, logger } from "owa-micro-common";
...
userRouter.post("/updateUser", async (req, res) => {
  var userId = decryptPayload(req.body.userId);
  ...
  var user = new UpdateUser(userBody);
  let key = `LOGGED_IN_USER_DETAILS_${reqUser.user_name}`;
  if (!userId || !user) {
      return res
          .status(STATUS.BAD_REQUEST)
          .send(
              `{"errorCode":"USRPRF0014", "error":"${ERRORCODE.USRPRF0014}"}`
          );
  }

  const {
      error
  } = validateUpdateUsers(user);
  if (error) {
      if (
          error.details != null &&
          error.details != "" &&
...
userRouter.post("/verifyMobileNumber", async (req, res) => {

  let phoneNo = decryptPayload(req.body.mobile_number);
  if (phoneNo != null && phoneNo.length != 10) {
      return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRMOB0000", "error":"${ERRORCODE.USRMOB0000}"}`);
  }
...
userRouter.post("/verifyForgotPasswordOtp", async (req, res) => {
  ...
  if (otp != null && otp.toString().length != 6) {
      return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF0003", "error":"${ERRORCODE.USRPRF0003}"}`);
  } else {
      ...
      if (redisResult && redisResult.length > 0 && redisResult[0]) {
          ...
          if (otpData.otp == otp) {
              ...
              return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF0005", "error":"${ERRORCODE.USRPRF0005}"}`);
          }
      } else {
          return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF0005", "error":"${ERRORCODE.USRPRF0005}"}`);
      }
...
userRouter.post("/resetPassword", async (req, res) => {
  ...
  let newPassword = decryptPayload(req.body.new_password);
  if (!txnId) {
      return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF00027", "error":"${ERRORCODE.USRPRF00027}"}`);
  }

  if (!newPassword) {
      return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF00019", "error":"${ERRORCODE.USRPRF00019}"}`);
  }
  if (!confirm_password) {
      return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF00028", "error":"${ERRORCODE.USRPRF00028}"}`);
  }

  if (confirm_password != newPassword) {
      return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF00029", "error":"${ERRORCODE.USRPRF00029}"}`);
  }
  ...
  if (redisResult && redisResult.length > 0 && redisResult[0]) {
      ...
      userService.getProfileDtlsbyMob(otpData.mobile_no, (err, result) => {
          ...
          if (result && result[0]) {
                      ...
                      bcrypt.compare(newPassword, lastPassword, async (err, res1) => {
                          if (res1) {
                              res
                                  .status(STATUS.BAD_REQUEST)
                                  .send(
                                      `{"errorCode":"USRPRF0004", "error":"${ERRORCODE.USRPRF0004}"}`
                                  );
                          } else {
              ...
              return res.status(STATUS.BAD_REQUEST).send(`{ "errorCode": "USRPRF0001", "error": "${ERRORCODE.USRPRF0001}" }`);
          }
      });
  } else {
      return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF00030", "error":"${ERRORCODE.USRPRF00030}"}`);
  }
});

userRouter.get("/getLoggedInUserInfo", async (req, res) => {
...
userRouter.post("/updatePassword", async (req, res) => {
  ...
  var userId = decryptPayload(req.body.userId);
  // var userId = req.body.userId;
  req.body.currentPassword = decryptPayload(req.body.currentPassword);
  req.body.newPassword = decryptPayload(req.body.newPassword);

  if (!userId) {
      return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF0001", "error":"${ERRORCODE.USRPRF0001}"}`);
  }
  if (!req.body.newPassword) {
      return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF0006", "error":"${ERRORCODE.USRPRF0006}"}`);
  }
  userService.getProfileDtls(userId, (err, result) => {
      if (result && result.length > 0) {
          ...
          bcrypt.compare(req.body.currentPassword, result[0].current_password, async (err, res2) => {
              if (!res2) {
                  res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF0007", "error":"${ERRORCODE.USRPRF00020}"}`);
              } else {

                  bcrypt.compare(req.body.newPassword, result[0].current_password, async (err, res1) => {
                      if (res1) {
                          res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF0008", "error":"${ERRORCODE.USRPRF00021}"}`);
                      } else {
...
//update user status 
userRouter.post("/updateUserStatus", async (req, res) => {
  try {
      ...
      if (updateUserStatus.user_id === 0) {
          return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF00031", "error":"${ERRORCODE.USRPRF00031}"}`);
      }

      const userStatus = await userService.getUserDetails(updateUserStatus.user_id);
      if (userStatus.length === 0) {
          return res.status(STATUS.BAD_REQUEST).send(`{"errorCode":"USRPRF00032", "error":"${ERRORCODE.USRPRF00032}"}`);
      }
...
userRouter.post("/sendOTPUpdateMobileNumber", async (req, res) => {
  var phoneNo = req.body.newMobileNumber;
  var oldphoneNo = req.body.oldMobileNumber;
  const userData = req.plainToken;

  if (userData.user_name == phoneNo) {
      return res.status(STATUS.BAD_REQUEST).send(`{ "errorCode": "USRPRF00017", "error": "${ERRORCODE.USRPRF00017}" }`);
  }

  userService.checkIfExist(phoneNo, async (err, userres) => {
      if (userres > 0) {
          return res.status(STATUS.BAD_REQUEST).send(`{ "errorCode": "USRPRF00016", "error": "${ERRORCODE.USRPRF00016}" }`);
      } else {
          let otp = Math.floor(100000 + Math.random() * 900000);
          var otpData = {
...
userRouter.post("/updateMobileNumber", async (req, res) => {
  const userData = req.plainToken;
  ...
  if (!req.body.newMobileNumber) {
      return res.status(STATUS.BAD_REQUEST).send(`{ "errorCode": "USRPRF0010", "error": "${ERRORCODE.USRPRF0010}" }`);
  }

  if (!req.body.oldMobileNumber) {
      return res.status(STATUS.BAD_REQUEST).send(`{ "errorCode": "USRPRF00025", "error": "${ERRORCODE.USRPRF00025}" }`);
  }

  if (!otp) {
      return res.status(STATUS.BAD_REQUEST).send(`{ "errorCode": "USRPRF0007", "error": "${ERRORCODE.USRPRF0007}" }`);
  }
  let changeMobileOtpKey = `MOBILE_CHANGE_OTP_${userData.user_name}`;
  let redisResult = await redis.GetKeyRedis(changeMobileOtpKey);
  if (redisResult && redisResult.length > 0 && redisResult[0]) {
      let otpData = JSON.parse(redisResult);
      if (otpData.otp != otp) {
          return res.status(STATUS.BAD_REQUEST).send(`{ "errorCode": "USRPRF0008", "error": "${ERRORCODE.USRPRF0008}" }`);
      } else {
          ...
          userService.selectUserToUpdateMobileNumber(username, (err, result) => {
              ...
              if (result[0]) {
                  ...
                  return res.status(STATUS.BAD_REQUEST).send(`{ "errorCode": "USRPRF0009", "error": "${ERRORCODE.USRPRF0009}" }`);
              }
          });
      }
  } else {
